...
WifiPhyTag::Deserialize (TagBuffer i)
{
  ...
  m_colorcode = i.ReadU8 ();
}

void
WifiPhyTag::Print (std::ostream &os) const
{
  os << m_wifiTxVector << " " << m_mpduType << " " << m_frameComplete << " " << m_colorcode;
}
...
WifiPhyTag::WifiPhyTag (WifiTxVector txVector, MpduType mpdutype, uint8_t frameComplete,uint8_t colorcode)
...
uint8_t 
WifiPhyTag::GetColorCode()
{
  //std::cout<<"tag colot "<< unsigned(m_colorcode)<<std::endl;
  return m_colorcode;
}
} // namespace ns3
